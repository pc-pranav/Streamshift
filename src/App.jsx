...
function StepPlaylists({ srcPlatform, srcAuth, onNext }) {
  ...
  const load = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      ...
      const data = await api(`/api/platform/playlists?${params}`, { headers: { Authorization: `Bearer ${token}` } });
...
function StepPreview({ srcPlatform, dstPlatform, srcAuth, dstAuth, playlistIds, playlists, includeLiked, onNext }) {
  ...
  async function run() {
    ...
    for (const job of jobs) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const data = await api(`/api/platform/tracks?${params}`, { headers: { Authorization: `Bearer ${token}` } });
    ...
    for (let i = 0; i < tracks.length; i += CHUNK) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          const data = await api("/api/core/match", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
function StepTransfer({ srcPlatform, dstPlatform, results, resolved, playlists, includeLiked }) {
  ...
  async function run() {
    ...
    for (const [plName, uris] of Object.entries(byPlaylist)) {
      ...
      try {
        const data = await api("/api/core/transfer", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
